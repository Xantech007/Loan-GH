<?php
// register.php - FINAL VERSION: Only 'members' table, no username column
session_start();
    ...
    if (!hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'] ?? '')) {
        ...
        if (empty($full_name) || empty($email) || empty($phone) || empty($password) || empty($confirm)) {
            ...
            if ($check->get_result()->num_rows > 0) {
                ...
                $stmt = $conn->prepare("
                    INSERT INTO members 
                        (full_name, email, phone, password, date_registered) 
                    VALUES (?, ?, ?, ?, NOW())
                ");
                $stmt->bind_param("ssss", $full_name, $email, $phone, $hash);
                $stmt->execute();

                // Get the auto-generated member_id
                $member_id = $conn->insert_id;

                // Format as MEM000001, MEM000123, etc.
                $display_id = "MEM" . str_pad($member_id, 6, "0", STR_PAD_LEFT);

                $success = "Account created successfully!<br>
                           <strong>Your Member ID: $display_id</strong><br>
                           Use this ID to <a href='login.php'>log in</a>.";
            }
        }
    }
}
